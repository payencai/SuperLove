...
public class BarrageView extends FrameLayout {
    ...
    private int displayLines = 3;//弹幕行数
    ...
    private void addView(BarrageViewBean barrageViewBean) {
        ...
        requestOptions.circleCrop().diskCacheStrategy(DiskCacheStrategy.ALL) .dontAnimate().override(100,100).error(R.mipmap.ic_launcher_round)
                .placeholder(R.mipmap.ic_launcher_round);
        Glide.with(getContext()).load(barrageViewBean.getHeadPictureUrl()).apply(requestOptions).into(iv);
        addView(itemView);
